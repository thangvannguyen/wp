...
function onlinemag_setup() {
	...
	add_image_size( 'onlinemag-main-slider', 765, 470, true );
	add_image_size( 'onlinemag-right-slide', 380, 220, true );
	...
	/* guternberg support, since 1.1.3 */
...
if( !function_exists('onlinemag_primary_menu_mobile_callback') ) :
	/**
...
function onlinemag_primary_menu_mobile_callback() {
	?>
		<ul id="mobile-menu">
			<li><a href="<?php echo esc_url( home_url( '/' ) );?>"><?php esc_html_e('Home','onlinemag');?></a></li>
			<li><a href="<?php echo esc_url( admin_url( '/nav-menus.php' ) );?>"><?php esc_html_e('Set Primary Menu','onlinemag');?></a></li>
		</ul>
	<?php
}
endif;
...
add_action( 'tgmpa_register', 'onlinemag_register_required_plugins' );
if ( ! function_exists( 'onlinemag_register_required_plugins' ) ) :
	/**
	 * Rise block plugin recommendation.
	 *
	 * @since 1.1.3
	 */
	function onlinemag_register_required_plugins(){
		$plugins = array(
			array(
				'name'     => esc_html__( 'Rise Blocks - A Complete Gutenberg Page builder', 'onlinemag' ),
				'slug'     => 'rise-blocks',
				'required' => false,
			),
		);
